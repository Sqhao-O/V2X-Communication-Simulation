...
TotalCap_robust = zeros(1, length(N_list));    % 总有效V2I容量（鲁棒，bps/Hz，信道实现平均）
TotalCap_nonrobust = zeros(1, length(N_list)); % 总有效V2I容量（非鲁棒，bps/Hz，信道实现平均）
...
for N_idx = 1 : length(N_list)
    ...
    while cntChannLs < channNum
        ...
        if n_robust > 0
            ...
            for pp = 1 : n_robust
                ...
                gk_actual = alpha_k_(k) * abs(hk_actual)^2;
                gmk_actual = alpha_mk_(m, k) * abs(hmk_actual)^2;
                actual_sinr = Pd_opt_robust(m, k) * gk_actual ...
                    / (sig2 + Pc_opt_robust(m, k) * gmk_actual);

                if actual_sinr >= gamma0  % V2V未中断
                    % 用实际功率重新计算V2I容量（C_mk可能含-infty惩罚值）
                    g_mB_actual = alpha_mB_(m) * abs(h_mB_(m))^2;
                    g_kB_actual = alpha_kB_(k) * abs(h_kB_(m, k))^2;
                    cap_actual = log2(1 + Pc_opt_robust(m, k) * g_mB_actual / ...
                        (sig2 + Pd_opt_robust(m, k) * g_kB_actual));
                    cap_sum_r = cap_sum_r + cap_actual;
                    v2v_success_r = v2v_success_r + 1;
                end
            end
            sum_cap_robust = sum_cap_robust + cap_sum_r;
            pair_count_robust = pair_count_robust + v2v_success_r;
        ...
        if n_nonrobust > 0
            ...
            for pp = 1 : n_nonrobust
                ...
                gk_actual = alpha_k_(k) * abs(hk_actual)^2;
                gmk_actual = alpha_mk_(m, k) * abs(hmk_actual)^2;
                actual_sinr = Pd_opt_nonrobust(m, k) * gk_actual ...
                    / (sig2 + Pc_opt_nonrobust(m, k) * gmk_actual);

                if actual_sinr >= gamma0  % V2V未中断
                    g_mB_actual = alpha_mB_(m) * abs(h_mB_(m))^2;
                    g_kB_actual = alpha_kB_(k) * abs(h_kB_(m, k))^2;
                    cap_actual = log2(1 + Pc_opt_nonrobust(m, k) * g_mB_actual / ...
                        (sig2 + Pd_opt_nonrobust(m, k) * g_kB_actual));
                    cap_sum_nr = cap_sum_nr + cap_actual;
                    v2v_success_nr = v2v_success_nr + 1;
                end
            end
            sum_cap_nonrobust = sum_cap_nonrobust + cap_sum_nr;
            pair_count_nonrobust = pair_count_nonrobust + v2v_success_nr;
    ...
    % 总有效V2I容量（总容量 / 信道实现次数 = 每次信道实现的平均总有效容量）
    TotalCap_robust(N_idx) = sum_cap_robust / channNum;
    TotalCap_nonrobust(N_idx) = sum_cap_nonrobust / channNum;
end

fprintf('\n===== V2V中断概率 =====\n');
fprintf('鲁棒:   '); fprintf('%.3e  ', P_outage_robust); fprintf('\n');
fprintf('非鲁棒: '); fprintf('%.3e  ', P_outage_nonrobust); fprintf('\n');
fprintf('\n===== 总有效V2I容量 (bps/Hz，每信道实现平均) =====\n');
fprintf('鲁棒:   '); fprintf('%.4f  ', TotalCap_robust); fprintf('\n');
fprintf('非鲁棒: '); fprintf('%.4f  ', TotalCap_nonrobust); fprintf('\n');

%% =================================================================
%%  保存仿真数据（供 thesis_figures.m 重绘）
%% =================================================================
save(fullfile(outputFolder, 'sim_03_data.mat'), ...
    'N_list', 'P_outage_robust', 'P_outage_nonrobust', ...
    'TotalCap_robust', 'TotalCap_nonrobust', 'p0', 'channNum');
...
% 坐标轴和标签
xlabel('车辆密度 N (辆)', 'FontName', FontName, 'FontSize', FontSize);
ylabel('V2V链路中断概率', 'FontName', FontName, 'FontSize', FontSize);
set(ax1, 'YScale', 'log');  % 对数坐标
ylim(ax1, [1e-3, 1]);  % Y轴范围调整为0.1%到1
xlim(ax1, [5, 50]);

% 图例
legend(ax1, [h_robust, h_nonrobust], ...
    {'鲁棒算法', '非鲁棒算法'}, ...
...
% 绘制容量曲线
plot(ax2, N_list, TotalCap_robust, '^-', 'LineWidth', LineWidth, ...
    'MarkerSize', MarkerSize + 2, 'MarkerFaceColor', [0.0 0.45 0.75], ...
    'Color', [0.0 0.45 0.75]);
plot(ax2, N_list, TotalCap_nonrobust, 'd--', 'LineWidth', LineWidth, ...
    'MarkerSize', MarkerSize + 2, 'MarkerFaceColor', [0.85 0.15 0.15], ...
    'Color', [0.85 0.15 0.15]);

xlabel('车辆密度 N (辆)', 'FontName', FontName, 'FontSize', FontSize);
ylabel('总有效V2I容量 (bps/Hz)', 'FontName', FontName, 'FontSize', FontSize);
ylim(ax2, [0, max(max(TotalCap_robust), max(TotalCap_nonrobust)) * 1.3]);
xlim(ax2, [5, 50]);
...
% 输出图像
setThesisFont(gcf);
print('-dpng', '-r600', fullfile(outputFolder, 'sim_03_V2V_Outage_and_V2I_Cap_vs_Density.png'));
print('-dpdf', '-r600', fullfile(outputFolder, 'sim_03_V2V_Outage_and_V2I_Cap_vs_Density.pdf'));
